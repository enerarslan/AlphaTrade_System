...
run_suffix="runD3_clean_ranker_seccorp_objectivepatch_core11_h12"
...
for arg in "$@"; do
  for blocked in "${blocked_overrides[@]}"; do
    if [[ "$arg" == "$blocked" ]]; then
      echo "Refusing objective-patch replay override for ${blocked}; edit the launcher intentionally instead." >&2
      exit 1
    fi
  done
done
