...
run_suffix="runE0_clean_xgboost_seccorp_core11_h12"
log_file="logs/train_wave1_xgboost_research_${run_suffix}.stdout.log"
model_name="wave1_xgboost_research_20260404_${run_suffix}"
...
for arg in "$@"; do
  for blocked in "${blocked_overrides[@]}"; do
    if [[ "$arg" == "$blocked" ]]; then
      echo "Refusing xgboost sec+corp replay override for ${blocked}; edit the launcher intentionally instead." >&2
      exit 1
    fi
  done
done
